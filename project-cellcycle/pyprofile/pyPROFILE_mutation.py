...
def load_polyphen(work_dir):
    ...
    polyphen = pd.read_table(base_folder+"/Common/pph2-full_all.txt", 
                         sep = '\t', 
                         header = 0)
...
def load_oncokb(work_dir):
    ...
    OncoKB = pd.read_table(base_folder+"/Common/allAnnotatedVariants.txt", 
                       sep = '\t', 
                       encoding = 'latin1')
...
def load_tsg(work_dir):
    ...
    base_folder=work_dir
    # Load 2020+ database
    OncoTSG = pd.read_table(base_folder+"/Common/2020_pancancer.csv",
                        sep=';', 
                        skiprows=1)
...
def mutation_assignment(mutation_data, database_dir):
    ...
    polyphen_oncogenes = polyphen.loc[polyphen['gene_symbol'].isin(oncogenes)]
    ...
    polyphen_tsg = polyphen.loc[polyphen['gene_symbol'].isin(tsg)]
